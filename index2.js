...
const quiz = [
    {
        ...
        correct: 'アメリカ',
        expla: 'McDonald RomanceのMVはアメリカのイベントSXSW（サウス・バイ・サウスウエスト）に参加し、ツアーを行ったときに撮影されました。とてもエモい曲になってます！'
    }, {
        ...
        expla: '強烈な表情で、爪痕を残すためにやったそうですw「キングヌー 井口 mステ」でググるとその写真が出てきますw'
...
let score = 0;
const explanation = [
    'キングヌークイズは全部で' + quizLength + '問あり、毎回ランダムで3問出題します!',
...
const setupQuiz = () => {
    ...
    document.title = "クイズアプリ：第" + problemNumber + '問';
    // console.log($problemNumber.textContent);
    // console.log($problemNumber);
    document.getElementById('js-question').textContent = quiz[question[quizIndex]].question;
    $correct.textContent = '現在' + score + '問正解';
    let buttonIndex = 0;
    while(buttonIndex < buttonLength){
        $button[buttonIndex].textContent = quiz[question[quizIndex]].answers[buttonIndex];
        buttonIndex++;
    }
};
...
const clickHandler = (e) => {
    if(quiz[question[quizIndex]].correct === e.target.textContent){
        ...
        window.alert('不正解！正解は「' + quiz[question[quizIndex]].correct + '」です。\n' + quiz[question[quizIndex]].expla);
    }
    // scrollTo(0, 0);
    window.scrollTo({
        top: 0,
